...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create status for current user when logged in" do
    sign_in users(:testuser)
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:testuser_2).id }
    end
  ...
  test "should get edit status for current user when logged in  ///////////////////////////////////////" do
    sign_in users(:testuser)
    get :edit, id: @status
    assert_response :success
  end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:testuser)
    put :update, id: @status, status: { content: @status.content, user_id: users(:testuser_2).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:testuser).id
  end
